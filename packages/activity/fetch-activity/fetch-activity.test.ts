...
import { ActivityConfig, Activity, ActivityError, ActivityGuardrails } from '../types';
...
const parseError = (error: Error): ActivityError => {
...
describe('fetch-activity', () => {
  ...
  test.each<Case>([
    ...
    if (shouldThrow) {
      ...
      expect(result).toStrictEqual(expectedActivity);
    }
  });
});
